...
sd5 = Unary NP (Unary NP (Leaf N "man"))
...
labeledBrackets (Binary c sd1 sd2) = 
    "[" ++ show c ++ " " ++ labeledBrackets sd1 ++ " " ++ labeledBrackets sd2 ++ "]"
...
numNPs (Binary c sd1 sd2) = 
    if c == NP then 1 + numNPs sd1 + numNPs sd2 else numNPs sd1 + numNPs sd2

numViolations :: [GrammarRule] -> StrucDesc -> Int
numViolations g (Leaf c s) = if elem (End c s) g then 0 else 1
numViolations g (Unary c sd) = 
    if elem (UnaryStep c (categoryOf sd)) g then
        numViolations g sd 
    else 1 + numViolations g sd
numViolations g (Binary c sd1 sd2) = 
    if elem (BinaryStep c (categoryOf sd1) (categoryOf sd2)) g then 
        numViolations g sd1 + numViolations g sd2 
...
replace sd [] r = r
replace (Leaf c s) (n:ns) r = undefined
replace (Unary c sd) (n:ns) r = 
    if n == 0 then Unary c (replace sd ns r) else undefined
